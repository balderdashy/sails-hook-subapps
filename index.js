...
module.exports = function(sails) {
  ...
  return {
    ...
    routes: {
      before: {
        '_subapp': function handleVirtualSubappRoute(req, res) {
          // Get a reference to the subapp, then delete it from the request
          var app = req.app;
          delete req.app;
          // Set the URL to "originalUrl"
          req.url = req.originalUrl;
          // Save a reference to the subapp as req._sails
          req._sails = app;
          // Run the request through the subapp's private router (skipping another round
          // of body parsing, session handler, etc.)
          console.log(app.router._privateRouter.router.toString());
          app.router._privateRouter.router(req, res, function handleUnmatchedNext(err) {
            //
    ...
    configure: function() {
      ...
      sails.config.http.middleware.subApps = function(req, res, next) {
        ...
        if (!_.any(self.mountMap, function(app, url) {
      ...
      sails.router.route = function(req, res) {
        ...
        if (!_.any(self.mountMap, function(app, url) {
          var regex = new RegExp("^"+url+"($|\\/)");
          if (req.url.match(regex)) {
            // If so, preserve the original URL (without the mounting path)
            // and change the url to _subapp (note the lack of a /).
            req.originalUrl = req.url.replace(regex,'/');
            req.url = "_subapp";
            // Save a reference to the subapp in the request
            req.app = app;
            // Continue processing the request with the parent app's 
            // virtual router, which will run the bodyparses, session
            // handler, etc. before the _subapp route handler passes
            // it off to the subapp's router.
    ...
    loadSubapps: function (cb) {
      ...
      async.auto({
        ...
        subapps = _.reduce(results.subappsFolder, function(memo, module, identity) {
          if (sails.config[self.configKey][identity] === false) {return memo;}
    ...
    initialize: function(cb) {
      ...
      sails.after(eventsToWaitFor, function() {
        ...
        sails.modules.loadModels(function modulesLoaded(err, parentModels) {
          if (err) return cb(err);
          ...
          self.loadSubapps(function modulesLoaded (err, modules) {
            ...
            async.each(_.keys(modules), function(identity, cb) {
              ...
              // Attempt to map the "main" subapp connection to the default parent
              // app model connection if no other mapping is provided.
              config.connections = _.defaults(config.connections || {}, {
                main: sails.config.models.connection
              });
              ...
              config.config.session = _.defaults(config.config.session || {}, sails.config.session);
              delete config.config.session.store;
                  ...
                  return {
                    loadModels: function(cb) {
                      defaultModelLoader(function(err, modules) {
                        if (err) {return cb(err);}
                        _.each(modules, function(module, identity) {

                          // If the parent app's subapp config has a mapping for this model, use it
                          if (config.models && config.models[module.identity] && typeof config.models[module.identity] == 'string') {
                            modules[identity] = module = parentModels[config.models[module.identity]];
                          }
                          // If the subapp has the "useParentModel" flag set for this model, attempt to match it to a parent
                          // model with the same identity
                          else if (module.useParentModel && parentModels[module.identity]) {
                            modules[identity] = module = parentModels[module.identity]; 
                          }
                          // If the subapp model is not mapped to anything in the parent, and there's a tablename prefix config, 
                          // apply it
                          else if (config.db && config.db.tablename_prefix) {
                            module.tableName = config.db.tablename_prefix + (module.tableName || module.identity);
                          }
